...
blockOrStatement:
  ...
  | dynamicNewStatement
  | assignStatement2
  | { ParserSupport.IsMethodOrFunc(_input.LT(1)) != 0 }? functionCallStatement
  | statement
  | expressionStatement
  ;
...
parameterArg:
  ...
  | n=identifier AS ( CLASS typeName | datatypeComNative | datatypeVar ) { ParserSupport.DefVar($n.text); } # parameterArgAs
...
expressionTerm2:
  ...
  | // methodOrFunc returns zero, and the assignment evaluates to false, if
    ...
    { ParserSupport.IsMethodOrFunc(_input.LT(1)) != 0 }? fname=identifier parameterListNoRoot  # exprt2ParenCall
  ...
  | // We are going to have lots of cases where we are inheriting methods
    ...
    { ParserSupport.Class && ParserSupport.UnknownMethodCallsAllowed() }? methodname=identifier parameterListNoRoot # exprt2ParenCall2
...
field:
    ...
    { ParserSupport.FieldReference($id.text); }
  ;
...
varRecField:
    ...
    { _input.LA(2) != NAMEDOT && ParserSupport.IsVar(_input.LT(1).Text) }? field
...
record:
    ...
    { ParserSupport.RecordSemanticPredicate(_input.LT(1), _input.LT(2), _input.LT(3)) }? f=filn { ParserSupport.PushRecordExpression(_localctx, $f.text); }
  ;
...
blockLabel:
    ...
    { _input.LT(1).Type != ABLNodeType.FINALLY.Type }?
    identifier | FILENAME
  ;
...
filename:
    t1=filenamePart
    ( { ( _input.LA(1) != TokenConstants.EOF) && !ParserSupport.HasHiddenBefore(_input) }? t2=filenamePart )*
  ;
...
assignmentList: // SEMITRANSLATED
  ...
  | // We want to pick up record only if it can't be a variable name
    { _input.LA(2) == NAMEDOT || !ParserSupport.IsVar(_input.LT(1).Text) }?
    record
...
catchStatement:
    CATCH
    n=ID AS classTypeName { ParserSupport.DefVar($n.text); }
...
classTypeName:
    { ParserSupport.HasHiddenAfter(_input) }? CLASS typeName
  | typeName
  ;
...
classStatement:
    ...
    { ParserSupport.DefineClass($tn.text); }
    blockColon
    classCodeBlock
...
datatypeDll:
  ...
  | { ParserSupport.AbbrevDatatype(_input.LT(1).Text) == CHARACTER }? id=ID
  ;
...
datatypeVar:
  ...
  | // Assignment of datatype returns value of assignment, if non-zero, is a valid abbreviation.
    { ParserSupport.AbbrevDatatype(_input.LT(1).Text) !=0  }? id=ID
  | typeName
  ;
...
defBrowseDisplayItemsOrRecord:
    ...
    { ParserSupport.IsTableName(_input.LT(1)) }? recordAsFormItem
...
dataRelation:
    ...
    { if ($n.ctx != null) ParserSupport.DefVar($n.text); }
  ;
...
menuListItem:
    MENUITEM n=identifier menuItemOption* triggerPhrase? { ParserSupport.DefVar($n.text); }
  | SUBMENU n=identifier ( DISABLED | labelConstant | fontExpression | colorExpression )* { ParserSupport.DefVar($n.text); }
...
defineParameterStatementSub1:
    PARAMETER BUFFER bn=identifier FOR TEMPTABLE? bf=record PRESELECT? labelConstant? fieldsFields? { ParserSupport.DefBuffer($bn.text, $bf.text); }
  ;

defineParameterStatementSub2:
    TABLE FOR record ( APPEND | BIND | BYVALUE )* # defineParameterStatementSub2Table
  | TABLEHANDLE FOR? pn2=identifier ( APPEND | BIND | BYVALUE )* { ParserSupport.DefVar($pn2.text); } # defineParameterStatementSub2TableHandle
  | DATASET FOR identifier ( APPEND | BIND | BYVALUE )* # defineParameterStatementSub2Dataset
  | DATASETHANDLE dsh=identifier ( APPEND | BIND | BYVALUE )* { ParserSupport.DefVar($dsh.text); } # defineParameterStatementSub2DatasetHandle
  | pn=identifier defineParamVar triggerPhrase? { ParserSupport.DefVar($pn.text); } # defineParameterStatementSub2Variable
  | pn=identifier defineParamVarLike triggerPhrase? { ParserSupport.DefVar($pn.text); } # defineParameterStatementSub2VariableLike
  ;
...
defineTempTableStatement:
    ...
    { ParserSupport.DefTable($tn.text, SymbolScope.FieldType.TTABLE); }
    ( UNDO | NOUNDO )?
...
defTableBeforeTable:
    ...
    { ParserSupport.DefTable($i.text, SymbolScope.FieldType.TTABLE); }
  ;
...
defineWorkTableStatement:
    ...
    { ParserSupport.DefTable($tn.text, SymbolScope.FieldType.WTABLE); }
    NOUNDO?
    defTableLike?
    labelConstant?
    defTableField*
...
formatOption:
     AS datatypeVar { ParserSupport.DefVarInlineAntlr4(); }
  |  atPhrase
  |  ATTRSPACE
  |  NOATTRSPACE
  |  AUTORETURN
  ...
  |  LIKE { ParserSupport.DefVarInlineAntlr4(); } field
  |  NOLABELS
  |  NOTABSTOP
  |  PASSWORDFIELD
  |  validatePhrase
...
functionStatement:
    ...
    f=FUNCTION
    id=identifier { ParserSupport.FuncBegin($id.text, _localctx); }
...
functionParam:
    ...
    { if ($bn.ctx != null) ParserSupport.DefBuffer($bn.text, $bf.text); }
...
functionParamStd:
    n=identifier AS asDataTypeVar extentPhrase? { ParserSupport.DefVar($n.text); } # functionParamStandardAs
  | n2=identifier likeField extentPhrase? { ParserSupport.DefVar($n2.text); } # functionParamStandardLike
  | { _input.LA(2) != NAMEDOT }? TABLE FOR? record APPEND? BIND? # functionParamStandardTable
  | { _input.LA(2) != NAMEDOT }? TABLEHANDLE FOR? hn=identifier APPEND? BIND? { ParserSupport.DefVar($hn.text); } # functionParamStandardTableHandle
  | { _input.LA(2) != NAMEDOT}? DATASET FOR? identifier APPEND? BIND?  # functionParamStandardDataset
  | { _input.LA(2) != NAMEDOT}? DATASETHANDLE FOR? hn2=identifier APPEND? BIND? { ParserSupport.DefVar($hn2.text); }  # functionParamStandardDatasetHandle
...
interfaceStatement:
    ...
    { ParserSupport.DefInterface($name.text); }
    classCodeBlock
    interfaceEnd
    statementEnd
  ;
...
methodStatement locals [ bool abs = false ]:
    METHOD
    (  PRIVATE
    |  PROTECTED
    |  PUBLIC // default
    ...
    id=newIdentifier
    functionParams
    ( { $abs || ParserSupport.Interface }? blockColon // An INTERFACE declares without defining, ditto ABSTRACT.
    | { !$abs && !ParserSupport.Interface }?
      blockColon
...
onStatement:
    ...
    | eventList
      ...
      |  { ParserSupport.AddInnerScope(_localctx); } blockOrStatement { ParserSupport.DropInnerScope(); }
      )
    )
  ;
...
onAssign:
    ASSIGN OF field triggerTableLabel?
       ...
       | { ParserSupport.AddInnerScope(_localctx.Parent); } blockOrStatement { ParserSupport.DropInnerScope(); }
       )
  ;

onAssignOldValue:
    OLD VALUE? f=identifier defineParamVar? { ParserSupport.DefVar($f.text); }
  ;
...
onWriteOfDbObject:
    ...
    ( NEW BUFFER? n=identifier labelConstant? { ParserSupport.DefBuffer($n.text, $bf.text); } )?
    ( OLD BUFFER? o=identifier labelConstant? { ParserSupport.DefBuffer($o.text, $bf.text); } )?
  ;
...
triggerProcedureStatementSub2:
    ( WRITE | REPLICATIONWRITE ) OF buff=record labelConstant?
           ( NEW BUFFER? newBuff=identifier labelConstant? { ParserSupport.DefBuffer($newBuff.text, $buff.text); } )?
           ( OLD BUFFER? oldBuff=identifier labelConstant? { ParserSupport.DefBuffer($oldBuff.text, $buff.text); } )?
  ;
...
usingStatement:
    ...
    usingFrom?
    statementEnd
    { ParserSupport.UsingState($tn.text); }
  ;
